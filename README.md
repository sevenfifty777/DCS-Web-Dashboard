...
- **Airboss Planner**: A dynamic toolkit for managing carrier operations, automatically syncing Wind Over Deck (WOD) and Base Recovery Course (BRC). Command any carrier to turn into the wind remotely, in any mission: the recovery controller is injected by the dashboard and no longer needs Foothold.
