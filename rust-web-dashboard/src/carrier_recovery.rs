...
/// Scripts for the telemetry poll behind `GET /api/airboss`.
...
mod tests {
    ...
    /// Minimal fake of the DCS mission scripting environment: one ship group,
    /// a wind field, a surface-type function, a manual clock with a scheduler,
    /// and message capture. Everything is reachable through the `Sim` table.
    const SIM: &str = r#"
Sim = {
  time = 0,
  wind = { x = 0, y = 0, z = 0 },
  ...
  routes = {},
  groupName = "CVN-71",
  ship = { x = 0, z = 0, headingDeg = 0, speedKt = 0, typeName = "CVN_71", exists = true },
  dict = {},
}
...
local unit = {}
function unit:isExist() return Sim.ship.exists end
function unit:getPoint() return { x = Sim.ship.x, y = 0, z = Sim.ship.z } end
function unit:getPosition()
  local h = math.rad(Sim.ship.headingDeg)
  return { p = self:getPoint(), x = { x = math.cos(h), y = 0, z = math.sin(h) } }
end
function unit:getVelocity()
  local h = math.rad(Sim.ship.headingDeg)
  local mps = Sim.ship.speedKt * 0.514444
  return { x = math.cos(h) * mps, y = 0, z = math.sin(h) * mps }
end
function unit:getTypeName() return Sim.ship.typeName end

local controller = { setTask = function(_, task) table.insert(Sim.routes, task) end }
local group = {}
function group:isExist() return Sim.ship.exists end
function group:getSize() return Sim.ship.exists and 1 or 0 end
function group:getUnit(i) return unit end
function group:getID() return 7 end
function group:getCoalition() return 2 end
function group:getController() return controller end

Group = { getByName = function(name)
  if name ~= Sim.groupName or not Sim.ship.exists then return nil end
  return group
end }
...
"#;
    ...
    fn module_loads_and_exposes_api() {
        ...
        for name in ["solve", "windData", "windReport", "start", "restore", "status", "monitor", "installMenus"] {
            assert!(
