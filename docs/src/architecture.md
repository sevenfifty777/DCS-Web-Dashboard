...
DCS-Web-Dashboard is composed of two primary components compiled into one executable (`rust-web-dashboard.exe`). There is no Node.js runtime or separate web server. Large replaceable assets are kept outside the executable in `images` and `icon` folders beside it to keep the binary smaller.
...
- **External Assets**: `/img/background.png` and `/icon/*` are served from the `images` and `icon` folders beside `rust-web-dashboard.exe`. Paths are resolved from the executable itself, so the parent folder can have any name and the process working directory does not affect asset loading.

> **Note:** The `web-dashboard` directory remains the frontend source project. The Rust crate embeds its built application output but excludes the external background and aircraft-icon assets.
